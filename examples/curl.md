...
```bash
curl -s http://localhost:4038/town | jq '.merchants[] | {name, mirrors, routes}'
```
...
Solana rail, use a Solana x402 client — or open `http://localhost:4038/` and let the payment
modal drive Phantom.)
...
curl -s "http://localhost:4038/hotel/search?checkIn=2026-09-01&checkOut=2026-09-03&guests=2" | jq '.offers[0]'
curl -s http://localhost:4038/store/catalog | jq '.items'
curl -s http://localhost:4038/health
...
```bash
# sandbox
curl -X POST http://localhost:4038/restaurant/book -d '{"date":"…","time":"19:00","party":2,"name":"Ada"}'
# the real thing
